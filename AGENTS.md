...
- hntv 官方频道优先级最高，公开源只补充官方没有的频道；分组输出顺序固定 河南卫视 -> 央视 -> 卫视 -> 自定义 -> B站直播
- **自定义流**（sources 表 type=custom，外部工具转流如抖音，见 `DOUYIN_LIVE.md`）：不参与探测，按名去重补充进聚合，分组「自定义」
- **聚合时探测过滤**（`prepare_public_channels` 阶段）：公开源频道宽松探测（200/206/403 可达），连续两轮失败才丢弃；hntv 官方源永不探测；失败记录跨轮持久化；过滤后的公开频道缓存到 `public_channels.json` 供官方源高频刷新复用
- **聚合互斥锁 + 首请求降级**：`_aggregate_lock` 保证同一时刻只有一个完整聚合；启动窗口期无缓存且后台聚合进行中时，`/api/live.m3u8` 秒级降级返回官方源列表（不重复全量聚合、不阻塞），后台完成自动切回完整列表
